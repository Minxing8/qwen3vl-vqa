...
def load_images(paths: List[str]) -> Tuple[List[Image.Image], List[bool]]:
    imgs, mask = [], []
    for p in paths:
        try:
...
def compute_confidence(scores, sequences, input_ids, tokenizer):
    ...
    - truncate at eos_token_id if exists
    - sum log_softmax(scores[t][i])[token] over steps
    ...
    for i in range(gen_ids.size(0)):
        ...
        if count == 0:
            ...
            confs.append(float(math.exp(logprob_sum / count)))
...
def main():
    ...
    # Match notebook: no seed unless user explicitly shuffles; seed only affects shuffling
    parser.add_argument("--seed", type=int, default=None, help="Shuffle seed (only used when --shuffle).")
    ...
    # Match notebook: confidence from generation token logprobs
    parser.add_argument("--output_confidence", action="store_true", help="Append (conf=...) at end of each answer.")
    ...
    # Match notebook: do NOT set do_sample. (defaults to False)
    gen_kwargs = dict(
    ...
    if args.output_confidence:
        gen_kwargs.update(dict(return_dict_in_generate=True, output_scores=True))
    ...
    for q_idx, question in enumerate(args.questions, start=1):
        ...
        with open(rank_csv, "w", newline="", encoding="utf-8") as fcsv, \
            ...
            for i in iterator:
                ...
                confidences = compute_confidence(
                    scores, sequences, inputs.input_ids, processor.tokenizer
                ) if args.output_confidence else [None] * len(answers)

                for p, ans, ok, conf in zip(batch_paths, answers, valid_mask, confidences):
                    ...
                    if conf is not None:
                        ans = f"{ans} (conf={conf:.4f})"
                    writer.writerow({"filename": p, "answer": ans})

                # del inputs, outputs, sequences, trimmed, answers, confidences, batch_imgs, messages, images # free up memory
                # torch.cuda.empty_cache() # free up memory
                # ===== improved cleaning =====
                del inputs, outputs, batch_imgs, messages, images
                if args.output_confidence:
                    del sequences, scores, trimmed, confidences
                
                # clean every 10 batches
